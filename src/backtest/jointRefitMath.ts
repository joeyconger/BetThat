...
function getComponentFields(game: GameForRating, key: ComponentParamKey): ComponentFields {
  switch (key) {
    ...
    case "errorCapPoints":
    case "varianceShrinkK":
      // Neither is one of JOINT_REFIT_COMPONENTS below (a post-loop
      // shrink/cap on the aggregate error or final rating, not a per-side
      // off/def differential component) -- this function is only ever
      // actually called with keys drawn from that fixed array, so these
      // cases are unreachable at runtime. Returning all-null (the existing
      // "no data for this component" signal, same as any other missing
      // field) rather than throwing keeps that guarantee from being
      // load-bearing.
